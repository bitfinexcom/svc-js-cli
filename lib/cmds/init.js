...
const usage = [
  'Usage: svc-js-cli init grenache-api-base <service-name> <port>',
  '',
  'Example: svc-js-cli init grenache-api-base bfx-util-net-js 1337'
].join('\n')

exports.cli = init
exports.cli.usage = usage
function init ([ stype, sname, sport ], cb) {
  if (!stype) {
    const err = new Error([
  ...
  initBaseGrenacheService(stype, sname, sport, conf, (err, data) => {
    if (err) return cb(err)
  })
}

function initBaseGrenacheService (stype, sname, sport, conf, cb) {
  sport = +sport
  ...
  spawnCmd('git', [ 'remote', 'add', 'upstream', conf.base ], { cwd: serviceHome })
  svcJs.log.info('', `Added ${conf.base} as upstream to git repo`)
...
function getServiceNaming (serviceArg) {
  ...
  const rootSource = path.join(__dirname, '..', '..', '..')
