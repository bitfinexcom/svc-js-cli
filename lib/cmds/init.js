...
function initBaseGrenacheService (stype, sname, sport, sroot, conf, cb) {
  ...
  const copyCmd = [ 'clone', rootSource, serviceHome ]
  spawnCmd('git', copyCmd)
  svcJs.log.info('', `Done!`)

  spawnCmd('git', ['remote', 'remove', 'upstream'], { cwd: serviceHome })
  spawnCmd('git', ['remote', 'remove', 'origin'], { cwd: serviceHome })

  spawnCmd('git', [ 'remote', 'add', 'upstream', conf.upstream ], { cwd: serviceHome })
  spawnCmd('git', [
    'remote', 'add', 'origin', 'https://github.com/bitfinexcom/' + sname
  ], { cwd: serviceHome })
  ...
  const devDeps = [
    'https://github.com/bitfinexcom/bfx-svc-test-helper'
  ]
  spawnCmd('npm', [ 'i', '--save-dev' ].concat(devDeps), { cwd: serviceHome })
  ...
  const exampleReadmeSubst = exampleReadmeTxt
    ...
    .replace(/__CONFIG__/ig, configFileName)
    .replace(/__UPSTREAM__/ig, conf.upstream)
  ...
  const sConfigContent = { grcServices: [ serviceLookupKey ] }
  const sConfig = JSON.stringify(sConfigContent, null, '  ')
  fs.writeFileSync(path.join(configRootTarget, configFileName + '.example'), sConfig, 'utf8')
  ...
  const exampleApiWrkSubst = exampleApiWrkTxt
    ...
    .replace(/__CONFIG_2__/ig, configFileName.split('.')[1])
  ...
  fs.mkdirSync(path.join(serviceHome, 'test'))
  const exampleTestFile = path.join(rootTemplates, 'integration.js.tmpl.js')
  const exampleTestTxt = fs.readFileSync(exampleTestFile, 'utf8')

  const exampleTestSubst = exampleTestTxt
    .replace(/__WORKERNAME__/ig, workername)
    .replace(/'__PORT__'/ig, sport)
  fs.writeFileSync(path.join(serviceHome, 'test', 'integration.js'), exampleTestSubst, 'utf8')
...
function getServiceNaming (serviceArg, sroot) {
  ...
  let workername = servicename
    .replace(/^bfx-/, 'wrk-')
    .replace(/-js$/, '-api')

  if (!/^wrk-/.test(workername)) {
    workername = 'wrk-' + workername
  }

  if (!/-api$/.test(workername)) {
    workername = workername + '-api'
  }
  ...
  let baseNamesTmp = servicename
    .replace(/^bfx-/, '')
    .replace(/-js$/, '-api')

  if (!/-api$/.test(baseNamesTmp)) {
    baseNamesTmp = baseNamesTmp + '-api'
  }

  baseNamesTmp = baseNamesTmp.split('-')

  const baseClassFileName = baseNamesTmp[1] + '.' + baseNamesTmp[0] + '.js'

  const configFileName = baseNamesTmp[1] + '.' + baseNamesTmp[0] + '.json'
